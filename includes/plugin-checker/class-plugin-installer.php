...
if ( !class_exists( 'MP_CORE_Plugin_Installer' ) ){
	class MP_CORE_Plugin_Installer{
		 ...
		 * @see      wp_parse_args()
		 * @param    array $args {
		 *      This array holds innavigationation the plugin
		...
		public function mp_core_install_plugin() {
			...
			if ( !empty( $this->_args['plugin_licensed'] ) && $this->_args['plugin_licensed'] ){
				...
				if ( !$license_valid ) {
					...
					//output navigation to try license
			...
			if (false === ($creds = request_filesystem_credentials($url, $method, false, false) ) ) {
				...
				// but have just produced a navigation for the user to fill in, 
				// so stop processing for now
				
				return true; // stop the normal page navigation from displaying
			}
