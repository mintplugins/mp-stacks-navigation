...
if ( !class_exists( 'MP_CORE_Plugin_Checker' ) ){
	class MP_CORE_Plugin_Checker{
		 ...
		 * @see      wp_parse_args()
		 * @param    array $args {
		 *      This array holds innavigationation for multiple plugins. Visit link for details.
		 *		@type array {
		 *      	This array could be in here an unlimited number of times and holds Plugin innavigationation. Visit link for details.
		...
		public function mp_core_create_pages(){
			...
			foreach ( $this->_args as $plugin_key => $plugin ){
				...
				if ( $plugin['plugin_wp_repo'] ){
					...
					$api = plugins_api( 'plugin_innavigationation', $args );					
		 ...
		 * @see      get_option()
		 * @see      wp_nonce_url()
	 	 * @param    array $plugins This has the same navigationat as the $args plugin in the construct function of this class
		 ...
		 * @see      sanitize_title()
		 * @see      wp_nonce_field()
	 	 * @param    array $dismiss_args This array holds Plugin innavigationation matching the second array in the construct function
		 * @return   void
		 */
		 public function mp_core_dismiss_button( $dismiss_args ){
			...
			if ($dismiss_args['plugin_required'] == false){
				echo '<navigation id="mp_core_plugin_checker_close_notice" method="post" style="display:inline-block; margin-left:.7em;">
		 ...
		 * @see      wp_verify_nonce()
		 * @see      update_option()
	 	 * @param    array $close_args This array holds Plugin innavigationation matching the second array in the construct function
		 * @return   void
		 */
